...
local CachedParent = (gethui and gethui()) or (game:GetService("CoreGui"):FindFirstChild("RobloxGui") and game:GetService("CoreGui")) or LocalPlayer:WaitForChild("PlayerGui")
...
function AuraPro:CreateWindow(Config)
    ...
    if SaveConfigEnabled and readfile and isfile and isfile(ConfigFileName) then
        pcall(function()
            SavedData = game:GetService("HttpService"):JSONDecode(readfile(ConfigFileName))
        end)
    end
    ...
    local function SaveCurrentConfig()
        if SaveConfigEnabled and writefile then
            pcall(function()
                writefile(ConfigFileName, game:GetService("HttpService"):JSONEncode(SavedData))
            end)
        end
    end
    ...
    local function UpdateScale()
        ...
        if Camera then
            ...
            if ViewSize.X < 600 then
                UIScale.Scale = ViewSize.X / 620
            else
                UIScale.Scale = 1
            end
        end
    ...
    function AuraPro:Notify(NotifConfig)
        ...
        local NTitle = NotifConfig.Title or "Thông Báo"
        ...
        ContentLbl.Size = UDim2.new(1, -16, 0, 26)
        ...
        task.delay(NTime, function()
            local TweenOut = TweenService:Create(Card, TweenInfo.new(0.3, Enum.EasingStyle.Quad, Enum.EasingDirection.In), {Position = UDim2.new(1, 300, 0, 0)})
            TweenOut:Play()
    ...
    MainFrame.Size = UDim2.new(0, 432, 0, 288)
    ...
    if Config.KeySystem and Config.KeySystem.Enabled then
        ...
        KeyCard.Position = UDim2.new(0.5, -160, 0.5, -100)
        ...
        SubmitBtn.MouseButton1Click:Connect(function()
            ...
            if InputtedKey == (KeyConfig.Key or "") then
                AuraPro:Notify({Title = "Thành công", Content = "Kích hoạt Key thành công!"})
                TweenService:Create(KeyOverlay, TweenInfo.new(0.3), {BackgroundTransparency = 1}):Play()
                KeyOverlay:Destroy()
            else
                AuraPro:Notify({Title = "Lỗi", Content = "Key nhập vào không chính xác!"})
            end
        end)
        ...
        GetKeyBtn.MouseButton1Click:Connect(function()
            ...
            if setclipboard and Link ~= "" then
                setclipboard(Link)
                AuraPro:Notify({Title = "Clipboard", Content = "Đã copy link lấy key!"})
            end
        end)
    end
    ...
    PagesArea.Size = UDim2.new(1, -150, 1, -70)
    PagesArea.Position = UDim2.new(0, 140, 0, 60)
    ...
    function WindowObj:CreateTab(TabName)
        ...
        function TabElements:CreateParagraph(Config)
            ...
            ParaFrame.Size = UDim2.new(0.98, 0, 0, 54)
            ...
            PContent.Size = UDim2.new(1, -16, 0, 28)
        ...
        function TabElements:CreateButton(Config)
            ...
            Btn.MouseButton1Click:Connect(function()
                TweenService:Create(Btn, TweenInfo.new(0.08), {Size = UDim2.new(0.95, 0, 0, 32)}):Play()
                task.wait(0.08)
                TweenService:Create(Btn, TweenInfo.new(0.08), {Size = UDim2.new(0.98, 0, 0, 34)}):Play()
                pcall(Callback)
            end)
        end
        ...
        function TabElements:CreateToggle(Config)
            ...
            ToggleBtn.Size = UDim2.new(0.98, 0, 0, 34)
            ...
            SwitchPin.Size = UDim2.new(0, 12, 0, 12)
            SwitchPin.Position = Toggled and UDim2.new(1, -14, 0.5, -6) or UDim2.new(0, 2, 0.5, -6)
        ...
        function TabElements:CreateSlider(Config)
            ...
            SliderFrame.Size = UDim2.new(0.98, 0, 0, 46)
        ...
        function TabElements:CreateDropdown(Config)
            ...
            DropFrame.Size = UDim2.new(0.98, 0, 0, 34)
            ...
            for _, OptName in ipairs(Options) do
                ...
                OptBtn.MouseButton1Click:Connect(function()
                    ...
                    Expanded = false
                    TweenService:Create(DropFrame, TweenInfo.new(0.2), {Size = UDim2.new(0.98, 0, 0, 34)}):Play()
                    Arrow.Text = "▼"
            ...
            HeaderBtn.MouseButton1Click:Connect(function()
                ...
                if Expanded then
                    TweenService:Create(DropFrame, TweenInfo.new(0.2), {Size = UDim2.new(0.98, 0, 0, 34 + (#Options * 26))}):Play()
                    Arrow.Text = "▲"
                else
                    TweenService:Create(DropFrame, TweenInfo.new(0.2), {Size = UDim2.new(0.98, 0, 0, 34)}):Play()
                    Arrow.Text = "▼"
        ...
        function TabElements:CreateTextbox(Config)
            ...
            local Placeholder = Config.Placeholder or "Nhập..."
            local Callback = Config.Callback or function() end

            local BoxFrame = Instance.new("Frame", PageFrame)
            BoxFrame.Size = UDim2.new(0.98, 0, 0, 34)
            ...
            Input.Size = UDim2.new(0.42, 0, 0, 24)
            Input.Position = UDim2.new(0.55, 0, 0.5, -12)
            Input.BackgroundColor3 = SelectedTheme.Card
            Input.PlaceholderText = Placeholder
            Input.Text = ""
            Input.TextColor3 = SelectedTheme.Text
            Input.TextSize = 11
        ...
        function TabElements:CreateKeybind(Config)
            ...
            BindFrame.Size = UDim2.new(0.98, 0, 0, 34)
            ...
            BindBtn.Position = UDim2.new(1, -78, 0.5, -11)
            BindBtn.BackgroundColor3 = SelectedTheme.Card
        ...
        function TabElements:CreateButtonGroup(Config)
            Config = Config or {}
            local GroupFrame = Instance.new("Frame", PageFrame)
            GroupFrame.Size = UDim2.new(0.98, 0, 0, 34)
            ...
            local Count = #Buttons
            local BtnWidth = (1 / Count) - 0.01

            for _, BtnConfig in ipairs(Buttons) do
                local BName = BtnConfig.Name or "Btn"
